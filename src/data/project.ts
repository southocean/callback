...
export const tasks: Task[] = [
...

  {
    ...
    detail: {
      why: 'Nam: "Put a password or answer to unlock => lets put back to Restricted, and have a bigger title in font size" and, restoring the line N204 trimmed: "Answer 20 correctly to unlock."',
        ...
        'The placeholder reads Password or answer, which is where that word lives now',
      ],
      raised: 'Nam, QA 30 Aug',
      notes: 'HALF OF N204 SURVIVED AND HALF DID NOT, and the half that did is the one that deleted a row. Moving the instruction into the title was the other half, and it was wrong in the way that is only obvious once it is on screen: a dialog titled with its own instructions has stopped having a title. The size gave it away first -- 17px was never a design decision, it was the size a six word heading had to shrink to in order to fit beside a lock and a close button, and a heading sized by its own length is not a heading any more.\\n\\nTHE SIZE IS THE SPEC PANEL, NOT A NEW NUMBER. .dp-title h1 is the heading of the dialog this one opens from: 400 22px/28px with -.2px tracking, measured. Two dialogs in one family get one heading treatment, so there was nothing to choose. Nam asked for bigger and the house already had the value.\\n\\nWHAT WENT WITH IT, and it is a real cost rather than a tidy one: "password" now appears nowhere a reader can see. It survives in the field accessible name and by implication in the progress line, which promises that answering is A way in rather than the only one. That word was doing work -- it was what made typing a cheat code a plausible thing to do in this box -- and the counter-argument is that a padlock over a single text field IS a password prompt to anybody who has ever seen one, so the fiction rests on the shape rather than on the label. Flagged rather than argued, and then answered the same day: Nam read the note and said "Password or answer sounds good, do that". So the placeholder carries it. That is the cheapest place it could have gone -- no row, no second sentence, and it sits in the field at the moment somebody is deciding what to type -- and it matches the accessible name, which never stopped saying both. Worth noting for anybody reading the sequence: the word travelled from a lead line, to a title, to a placeholder, and got smaller every time while staying exactly as legible. The first two placements were both trying to make a room say what it was, and the placeholder is the only one of the three that says it where the decision is actually made.',
    },
  },

  {
    id: 'N209', col: 'review', size: 'S', tag: 'specs',
    title: 'BUG: a rebuild serves the old entry against the new chunks',
    note: 'It reads as lost work. It is the previous build importing files this one deleted.',
    detail: {
      why: 'Nam: "a new rebuild has fucked up all our changes so far ... I also want to avoid this problem, wherever I do things, then rebuild then my changes are gone. How are we rebuilding this client? how come we are lacking info?"',
      done: [
        'Nothing was ever lost: the browser was running the previous build',
        'The dev server sends no-store, so a rebuild is always what you see next',
        'The built page stamps its own fingerprint onto app.js and styles.css',
      ],
      raised: 'Nam, mid QA',
      notes: 'NOTHING WAS LOST, AND THE REAL FAULT IS WORSE THAN STALENESS. The chunks are content hashed and look after themselves. app.js, boot.js, styles.css and index.html are NOT -- they are named for what they are, which is correct for an entry point and is exactly what makes this happen.\n\nSo: a browser holds a cached app.js from the previous build. That file imports chunk hashes belonging to that build. And build.mjs deletes every old chunk on the way in, deliberately, so that the committed output does not accumulate dead files forever. Those URLs are therefore GONE. The imports 404, and the screen either stops updating or stops working -- which looks exactly like the last hour of changes having been destroyed.\n\nIt is not a stale page. It is the previous build running against the current build\'s chunk names, which is a state no amount of waiting recovers from.\n\nTWO FIXES, FOR TWO DIFFERENT AUDIENCES. The dev server now sends no-store on everything, which ends it locally and completely: a rebuild is always what you see next, with no hard reload and no devtools setting. No-store rather than no-cache, because no-cache still stores and revalidates and there is nothing here to revalidate against.\n\nAnd the build stamps a fingerprint of the entry plus the stylesheet into their own URLs, so a returning visitor to the published site cannot be handed a cached entry that points at deleted chunks. It changes when the contents change and not otherwise, so it costs nothing on a build that changed nothing.\n\nWHAT THIS DOES NOT FIX, and it is worth knowing: index.html itself is still served under whatever cache policy the host applies. GitHub Pages revalidates HTML quickly, so the window is small, but it is not zero. The complete answer is a content hash in the entry filename, which is a larger change to make for a window measured in minutes.',
    },
  },

  {
    id: 'N210', col: 'review', size: 'M', tag: 'onboarding',
    title: 'BUG: the stork had a plan and no elevation',
    note: 'Its body was a stack of flat discs. A helper that draws horizontal rings drew them.',
    detail: {
      why: 'Nam: "the bird is not right, it should be a stork with shorter neck, thicker body and the bag should be like a fabric bag, more round on the bottom and gathered on top by the stork\'s beak. The current bag looks more like a lantern ... and the stork\'s body is literally flat, there is no body to the bird, very bad." Plus: "to make it more immersive, we can add some clouds in the background with the bird."',
      done: [
        'The body is a solid of revolution about the spine, not a stack of flat discs',
        'Short thick neck and a deep barrel, which is a stork rather than a heron',
        'The bundle is pinched at the tie and spherical below, like tied cloth',
        'Clouds sit behind it in depth and drift the other way',
        'The programme is called Culture',
      ],
      raised: 'Nam, QA of the Culture programme',
      notes: 'LITERALLY IS THE CORRECT WORD, and the cause hid behind a helper that does almost the right thing. hoop() draws a ring in the HORIZONTAL plane -- it varies x and z at a constant y -- which is exactly right for a lantern, a basin, a pagoda eave, and every other thing that had used it. The bird\'s spine runs along x, so a cross section of it has to vary y and z instead. Feeding the body to hoop() produced a stack of flat discs threaded along the spine, every one of them at y = 0. The bird had a plan and no elevation.\n\nIt survived because it is not WRONG anywhere: the call is well formed, the helper does what it says, the node count is right, the density measurement is fine, and nothing in this repository renders. A bird made of horizontal discs measures identically to a bird made of vertical ones.\n\nWITH REAL VOLUME AVAILABLE IT WAS WORTH USING. A stork is a heavy bird: short thick neck held straight out at about half the body length, a deep barrel widest a third of the way back, and a long straight beak that is the one thin thing on it. The contrast is what makes everything else read as bulk, which is why the previous version -- long thin neck, thin body -- read as a heron and not as the bird in the reference.\n\nTHE BAG WAS A LANTERN AND THE PROFILE SAYS WHY. It swelled widest at two fifths of the way down and closed again, which is a lantern in cross section. Tied cloth is PINCHED at the top and spherical below: widest at two thirds, rounding shut at the bottom, and the neck stays narrow for the first quarter. Raising the exponent inside the profile moves the peak down and holds the gather, which is the whole change.\n\nTHE CLOUDS ARE BEHIND IT IN DEPTH rather than drawn smaller. The perspective divide dims and shrinks them for free, which is the entire reason they read as distance instead of as decoration, and they drift the opposite way to the bird\'s heading, slowly, which is what sells the flying. Each is four overlapping lumps, because a cumulus is lumps and a single sphere is a balloon.\n\nAND MOTIFS IS NOW CALLED CULTURE, which is what the set has actually been since the roster settled: a lotus, sky lanterns, a carousel, a pagoda, a tall ship, a stork, a maypole, an advent star, a jellyfish, an armillary sphere and a fountain. The stored preference key still holds the old id, so anyone who had picked it lands on Geometry once and picks again.',
    },
  },
...

  {
    ...
    detail: {
      ...
      raised: 'Nam, before the release',
      notes: 'NOTHING WAS STALE, AND THE REASON IS STRUCTURAL RATHER THAN LUCKY. Every number a reader sees is derived at the point it is shown: the heatmap totals its own days, the board counts its own tickets, the engineering tab counts its own tests and checks, and the bundle sizes and commit hash are stamped into the HTML by build.mjs rather than typed into it. There is no number in this build that a person has to remember to update.\n\nThe two literal counts that turned up in a search -- 184 commits, 17 quests -- are both inside NOTES quoting what Nam said at the time. Those are a record of a conversation and are supposed to say what he said, not what is true now.\n\nCHECKED RATHER THAN ASSUMED, AND CHECKED AGAIN AT THE RELEASE: 232 commits, 257 tickets, 12 bugs, 21 quests, 5 clips, 197 tests, 17 layout assertions, and just over 31.1 kB gzipped against a 51,200 byte budget, about 39% still free. The one claim that is prose rather than arithmetic -- "17 assertions across both hosts" -- was run and prints exactly seventeen.\n\nTHE SIZE IS THE ONE THAT CANNOT BE QUOTED TO THE BYTE, and finding out why was the useful half of the recount. This note ships inside the bundle it describes, so writing an exact figure changes the figure: 31,188 became 31,191 the moment it was typed, then 31,190 when that was corrected, then 31,188 again. Gzip is sensitive to which digits it is compressing, and the sequence oscillates rather than settling. Every other number here is measured from outside the artefact and holds still; this one is a measurement taken from inside, so it is reported to the precision it actually has.\n\nThe recount is the point of writing them down. Three moved between the first pass and the tag -- 221 commits to 232, 246 tickets to 257, and the bundle by roughly 280 bytes across a mobile pass, a persona change and a caption rework -- and the other five did not, which is what a set of numbers looks like when only the ones that should change do.',
    },
  },
  ...
  {
    id: 'N246', col: 'review', size: 'M', tag: 'onboarding',
    title: 'The green room is measured against the real one and cut down for a phone',
    note: 'A level meter sitting on the microphone, three inert join methods, and four type deltas.',
    detail: {
      why: 'Nam: "please go in the original meet and measure everything in mobile view so we can mirror it faithfully" and then, from the real product on a real phone: "in mobile view they clean up a lot of unnecessary stuff. Companion mode and whatever extra join methods, those are only on desktop. On mobile you just join. And we have the 3 dot icon on the corner left overlapping the mic icon."',
      done: [
        'The reference captured at 390x844 in a signed-in browser: geometry, type and surfaces',
        'The input-level meter is gone below 600, where it had nowhere to stand',
        'Other ways to join and its three options are desktop only',
        'The prompt is 22/28 Google Sans, the name gets a 20px line, the round controls are 54x44 r22',
        'Switch account stops being blue',
        'The tools card fits the 320 floor',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'THE METER HAD NOWHERE TO STAND, and the arithmetic says so rather than an opinion. It is a 26px circle inset 16 from the tile left and bottom, which is clear of everything on a desktop. At 390 the tile is 250 wide and three 56px controls with 16px gaps come to 200, leaving 25 a side -- so 16+26 lands on top of the microphone. Three controls and a meter do not both fit at any position that still reads as Meet, so it is hidden rather than moved. It is also the honest thing to lose: an input-level meter on a page with no input has been three static dots since it was built.\n\nMOBILE MEET JUST JOINS, and the code already half-agreed. All three of Companion mode, presenting and dial-in are inert here, because they are three products this build is not cloning -- so a disclosure that opens onto three things that do nothing was costing the most space on the screen with the least to spare.\n\nWHAT THE MEASUREMENT METHOD CANNOT SEE, which matters more than any single number below. The capture is a DESKTOP user agent at a phone viewport, because meet.google.com sent an iPhone agent does not serve a small layout at all -- it redirects to an app-install page. So it measures Meet responsive CSS at 390px, which is the right reference for a web clone and is NOT the same thing as the native mobile app. Nam has the app and could see that it drops Companion mode and the extra join methods entirely, which the capture still showed. Geometry and type come from the capture; what to SHOW came from him.\n\nAND THE CAPTURE IS IN AN ABNORMAL STATE, recorded so it can be re-taken: the reference browser has camera and microphone denied, so the tile showed an Allow microphone and camera button where a preview belongs and two device chips rather than three. The 54x44 r22 control size is from that capture and disagrees with the 56x48 r24 this stylesheet measured by toggling them on the desktop product. Both are real measurements of different things; the phone one is applied below 600 and the desktop constant is untouched.\n\nTHE 320 OVERFLOW IS THE SAME AUTOMATIC MINIMUM AGAIN. The tools card is built from fixed parts that add to its 322px desktop width -- 36 of padding, 36 of icon and gap, a 162px text block, an 88px button -- and the card is clamped to calc(100% - 32px), so at 320 it becomes 288 and the fixed middle cannot give. The Start button ran 22px off the screen. min-width: 0 on the text block, which is the fourth time in this pass that a flex item refusing to shrink has been the whole bug.',
    },
  },

  {
    id: 'N247', col: 'review', size: 'M', tag: 'onboarding',
    title: 'The green room on a phone is a tile and a button',
    note: 'Screenshots of the real mobile app, which is a different screen from the responsive web one.',
    detail: {
      why: 'Nam, with screenshots of Meet on a phone: "there is still a lot for you to clean up in our mobile UI, we still have for example the video effect button, the caption option, etc. and heres if you click the 3 dot more option button. note that here the top only shows the avatar, not you@google.com bla bla. And if you click this avatar, we open up the how this is built screen in a popup screen just like if we were to click it in the home screen."',
      done: [
        'The bar is a wordmark and an avatar; the avatar opens the spec panel, as on home',
        'The effects control goes, which is what frees the level meter',
        'The overflow menu drops Backgrounds and effects with it',
        'The heading, the participant line and the tools card go',
        'The badge on Join now goes; the reference warns on the camera chip only',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'A SCREENSHOT BEAT THE MEASUREMENT, and it is worth being precise about why rather than filing it as taste. N246 measured the reference by pointing a DESKTOP user agent at a 390px viewport, having found that an iPhone agent on /home gets redirected to an app-install page -- and generalising that to the whole product, which was wrong. An iPhone agent on a MEETING URL serves the real mobile green room. /home redirects; the meeting does not. Nam: "thats why I want you to do it in the simulated mobile device from the browser developer tool, which then emulates an actual mobile device, not just the resolution." He is right, and the corrected capture is in N248. That measures Meet responsive CSS at phone width, which is a real thing and is NOT the screen a phone actually shows. The app drops Companion mode, the extra join methods, the effects control, the tools card, the heading and the account line; the responsive-web capture kept all of them. Geometry and type from the capture were sound. What to SHOW could only come from the app.\n\nAND IT CORRECTED THE FIX FROM THE TICKET BEFORE. N246 hid the level meter, reasoning that three 56px controls fill a 250px tile and leave its 16+26 inset no room. The arithmetic was right and the conclusion was wrong: the real phone keeps the meter exactly where it always is, bottom-left, and has only TWO controls beside it. Meet does not make room for the meter, it drops the effects button. So the overlap was never a placement problem -- it was a desktop control being carried onto a phone, and removing that control fixed the collision and the clutter at once.\n\nWorth keeping as a shape: when two things collide, ask whether one of them should be on the screen at all before moving either.\n\nTHE MENU FOLLOWS THE CONTROL. Backgrounds and effects is the menu entry FOR the effects button, so a phone that has dropped the button and kept the entry would be offering a feature the screen has just removed. Filtered when the menu opens rather than in CSS, because the separator is drawn by the item below it and hiding the first row in a stylesheet leaves the rule floating above nothing.\n\nBOTH BARS ARE IN THE MARKUP AND THE STYLESHEET PICKS. Building one or the other from a matchMedia call at render time would mean this screen looked different depending on the width it was first opened at, which is a bug waiting for a rotation.\n\nONE THING NOT CHANGED, because it could not be measured: the tile radius. Ours is 8. The reference capture put the background on an inner element with no radius of its own, so the rounding lives on a parent that the dump could not attribute, and the app screenshot looks rounder than 8 without being measurable from an image. Left alone rather than guessed at.',
    },
  },

  {
    id: 'N248', col: 'review', size: 'S', tag: 'specs',
    title: 'The mobile reference was reachable all along, and the radius was measurable',
    note: 'Two corrections Nam had to make. /home redirects; a meeting URL does not. And a PNG can be measured.',
    detail: {
      why: 'Nam: "thats why I want you to do it in the simulated mobile device from the browser developer tool, which then emulates an actual mobile device, not just the resolution - which you claimed to be the same! No its not the same, you are serving a desktop app on a mobile resolution, thats not the same as running a mobile emulation and getting the mobile served site." And: "what do you mean, you have the screenshot and you have your code, stack them on top of each other, change the radius around until it matches the overlay screenshot, you shouldnt even need my help there."',
      done: [
        'The reference is captured with a mobile user agent on a meeting URL, which serves the real mobile green room',
        'The tile is 254x422 at radius 28, measured off the pixels rather than guessed',
        'The wrong generalisation is corrected in tools/ref-mobile.mjs and in N247',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'A DEVICE PRESET IS NOT A RESOLUTION, and the earlier claim that it was is the root of a whole round of wrong measurements. DevTools device mode sets the user agent as well as the metrics, and the user agent is what decides which SITE the server sends. Pointing a desktop agent at a 390px viewport measures the desktop app reflowed narrow; it is not the mobile app, and treating the two as the same thing is what put the effects control, the tools card, the heading and the account line into a screen that has none of them.\\n\\nAND THE MOBILE PAGE WAS REACHABLE ALL ALONG. The first pass sent an iPhone agent to meet.google.com/home, got the app-install interstitial, and generalised that to the product. Wrong: /home redirects and a MEETING URL does not. The same agent on meet.google.com/<code> serves the real mobile green room -- captured now, and it is a different screen, with two device chips at 54x44 r22 on a 70px pitch, Join now at 240x56 r28, and nothing between the tile and that button.\\n\\nThe 54x44 r22 in N246 turns out to be right for the right reason rather than by luck: it was read off a desktop-agent capture in a blocked-permission state and flagged as suspect on both counts, and the true mobile page gives the same numbers.\\n\\nA RADIUS IS A MEASUREMENT, NOT A GUESS, and N247 should not have asked. The corner is a circle centred one radius in from each edge, so the inset of the dark pixels d rows below the top is R - sqrt(2Rd - d^2), which solves to R = (inset + d) + sqrt(2 * inset * d). Sampled at nineteen values of d and taken as a median it is stable to a third of a pixel, and run against OUR tile it returns 8.3 for a CSS value of 8 -- which is the control that makes the reference reading trustworthy.\\n\\nMeet: 28.5 with a spread of 27.2 to 29.6. Taken as 28, because that is the M3 radius this stylesheet already uses for the banner, the scheduled card, the bug dialog and Join now, and a measurement landing half a pixel off a number the design system already owns is a measurement of that number. Ours now measures 28.0 at 254x422, against the reference 254x422.',
    },
  },

  {
    id: 'N249', col: 'review', size: 'L', tag: 'trust',
    title: 'The visitor gets one avatar, one name, and a call that starts itself',
    note: 'Three screens had each invented an answer to "who is looking at this". Now there is one.',
    detail: {
      why: 'Nam: "the avatar is all over the place! In home screen, the avatar is a green G by default, but then if you get admin right then its A with more like orange avatar. Here in the greenroom the avatar is A with green background, but then on the call its NN with green avatar ... We need to fix the avatar such that we get avatar from the same place." Plus: the tile name should be You, the camera question should go, and Join should start the walkthrough rather than ask.',
      done: [
        'ui/avatar.ts is the only place the visitor is described: GG by default, AD with admin',
        'The home bar, the green-room bar, the green-room tile and the call solo tile all ask it',
        'The visitor is called You, on the tile and in the call',
        'The camera question is replaced; Join starts the walkthrough with no picker',
        'The tile drops its avatar entirely, because the line in it is Nam speaking',
        'A 24px gutter on the prompt, so it breaks earlier and clears the edges',
        'The overflow menu is narrower on a phone',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'THE DRIFT HAD A CAUSE, and it is the useful part of this ticket: there was no single answer to "who is looking at this", so three screens each invented one. The home screen owned the letter, the state class and the label together. The green room copied the LETTER and not the class, which is exactly why it showed the admin A on the default green. The call borrowed the CV subject initials for a tile that is not him. None of the three was wrong on its own terms; there was simply nowhere for them to agree.\\n\\nGG RATHER THAN G, for a reason worth keeping: one letter reads as somebody initial, which invites the question whose, and the answer is nobody -- it is the person holding the phone. Two letters read as a label. AD on the same logic.\\n\\nTHE SOLO TILE WAS THE SHARPEST CASE. It carries the mute badge, the raised hand and the input meter, three things that only make sense on the tile of the person holding the phone, and it was announcing itself as Nam Nguyen with his initials on it. Both people in the call were called Nam and the visitor appeared nowhere on their own screen. Naming them You also fixes who the hand belongs to: it is Nam, and it now has somebody to be talking TO.\\n\\nTHE CAMERA QUESTION CONTRADICTED A PROMISE THIS BUILD MADE. N-something removed getUserMedia entirely so the page can say it never asks for a camera, and then the green room asked whether you would like to be seen. Nam: "this is a pointless question since camera doesnt and shouldnt work anyway, so the question violates our initial claim and lose trust." A promise and a prompt that contradict it is worse than either alone. The line now spends itself warming up the densest screen in the build instead.\\n\\nAND THE PICKER GOES, WHICH REVERSES N188 ON A BETTER ARGUMENT THAN IT WAS BUILT ON. That ticket said a choice about being narrated at should be offered before the narration starts. What it did in practice was offer a stranger a decision they had no information for, and half of them chose the version of this CV that explains nothing about itself. Nam: "that option is very bad for us cause we dont get to show this CV in the best light." The way OUT is what makes this defensible rather than pushy, and it is unchanged: the captions control stops him at any moment and says so. An escape hatch a visitor can find beats a question they cannot answer yet.\\n\\nThe explore branch in call.ts is now unreachable and is left in place deliberately, so the behaviour change and the deletion can be reviewed apart. Removing it, and the lane memory in ui/lane.ts with it, is a follow-up.\\n\\nNO AVATAR ON THE TILE AT ALL, which is the second half of the same problem the name had. The tile is the VISITORS and the line in the middle of it is NAM talking, so a face above his sentence made the sentence look like theirs -- and it had the same fault before this pass for the opposite reason, when the initials were his and the tile looked like his too. Removed rather than swapped: a green room shows the person about to join and there is no camera to show them with, which is also what the reference does in this state. Applied at every width, because the reason is about who is speaking and that does not change with the viewport.\n\nONE NUMBER HERE IS AN ESTIMATE AND IS FLAGGED AS ONE. The menu is 212 on a phone rather than 225, taken from the proportion in Nam screenshot -- the reference menu is 0.835 of the tile it opens over, and the tile is 254. The run that was going to measure it directly could not get the mobile page to finish rendering. Everything else on this screen is measured; that one is worth re-taking.',
    },
  },

  {
    id: 'N250', col: 'review', size: 'L', tag: 'call',
    title: 'The call fits a phone, and the share is reachable and readable on one',
    note: 'A bar 468px wide on a 390px screen, carets drawn on the buttons they belong to, and a share nobody could read.',
    detail: {
      why: 'Nam: "the mobile view does not have screen sharing, so we will have to improvise here. Instead of sharing screen as a button on screen, we put it in the more option dot ... once you click share screen on mobile, we dont have the panel choosing the screen source anymore, but its a one click and default to sharing full screen." And: "many things are out of place, overflowing, breaking ... The space here is very small ... Theres barely any space to show anything."',
      done: [
        'The bar carries six controls and fits 390 and 320',
        'The device carets are gone; they were drawn on top of the buttons they open',
        'Share moves into the overflow and shares the whole screen in one press',
        'The share is 3:2 rather than 4:3, and the desktop is drawn at 80% into it',
        'The meeting code truncates instead of wrapping the top bar to two lines',
        'A share route in tools/qa-mobile.mjs, so the new flow is checked rather than assumed',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'THE BAR WAS 468 WIDE ON A 390 SCREEN, and 154 of it hung off the side at 320 -- which put LEAVE out of reach, on the one screen where being unable to stop is the worst failure available. Eight controls and two carets against the reference six.\\n\\nTHE CARETS WERE THE INTERESTING PART. They are the desktop audio and video device pickers, on a page that never opens a device, and they were positioned ON TOP of the buttons they belong to: a 44x48 overlap, so half of every microphone press was landing on a menu opener. The harness had been reporting that overlap for several passes and it read as a cosmetic near-miss rather than a mis-hit.\\n\\nAND `.chev { display: none }` HAD BEEN IN THE 720 BLOCK ALL ALONG, doing nothing, because `.unit > .chev` further down the file is (0,2,0) against its (0,1,0). Then the fix for THAT tied at (0,2,0) and lost on source order. Then the presenting pill cap did the same thing. Four times in one pass, which is enough to state as a rule: an override written earlier in the file than what it overrides has to buy a point of specificity, and the way to know it worked is to measure the winner rather than read the intent.\\n\\nSHARING IS AN IMPROVISATION AND IS LABELLED ONE. Mobile Meet cannot share at all, so there is no reference row to match; the menu is where it goes because it is the only place with room, and it goes at the top because on this build sharing is not a minor setting, it is how the CV gets shown. The picker goes with it: it exists to choose between three tabs, a window and the screen, and the screen is the only one of those with room to be read on a phone.\\n\\nTHE SHARE ITSELF WAS TWO PROBLEMS. The 4:3 cap is right on a desktop, where the wrap is wider than tall; on a 390 phone the wrap is about 390 by 600, so 4:3 handed the share 390 by 292 and threw away half the vertical space the phone has most of, to keep the shape of a monitor nobody is looking at. 3:2 gives it 585. Then a box 125% of that frame, drawn at 0.8, lays the desktop out at 487 by 731 and paints it into 390 by 585 -- so a window that used to be cropped to its title bar has room to be a window. Text gets smaller, which is the trade Nam called, and he is right that unreadable-and-cropped loses to small-and-whole.\\n\\nWHAT IS NOT DONE YET, stated rather than left to be discovered: the mock OS is zoomed but not TAILORED. The taskbar still crowds -- the weather sits under the tray icons and the clock under a pinned app -- windows can still open wider than the desktop, and the self tile overlaps the bottom of the share. Nam asked for the folders, the browser and the media player to be re-cut for a vertical display, and that is a design pass of its own rather than the tail of this one.',
    },
  },

  {
    id: 'N251', col: 'review', size: 'M', tag: 'call',
    title: 'The overflow menu opens on the screen, and carries what the phone puts there',
    note: 'It was never a dead button. The menu was mounting 496px past the right edge.',
    detail: {
      why: 'Nam: "clicking the ... button doesnt do anything! I already sent you a screenshot of when its expanded. Copy that thks. How else do we trigger the screensharing? Even the CC isnt in the mobile view either, so we gotta move that CC into the more option button too. We will start without the CC, and Nam will enable CC and start talking." Plus, on the tile menu: "this should open to the left to not going out of the screen, also remove the first option here on mobile, only 2 options left." Plus: "remove the restless bar on mobile, it doesnt add anything."',
      done: [
        'The overflow menu is placed on the screen at 390 and at 320',
        'Its rows are the reference mobile menu, plus the improvised share at the top',
        'Captions moves off the bar into that menu, leaving the reference six',
        'On a phone the walkthrough switches captions on itself before it speaks',
        'Every attachMenu menu is clamped into the viewport, both axes',
        'The tile menu drops Minimize, which is unreachable on a phone',
        'The restlessness meter does not appear on a phone',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'IT WAS NOT A DEAD BUTTON, AND THAT IS THE WHOLE LESSON. The handler ran, the menu built, mounted and painted -- at x 886 on a 390 screen. `.gm-pop.call-more { left: 886px }` is a real measurement off the desktop reference, where that button sits at about x 900 in a 1512 window. A hard-coded pixel constant is correct at exactly one viewport width, and it fails SILENTLY: an element parked 496px off the edge is indistinguishable, from the outside, from a click that does nothing. Two sessions of mobile QA never caught it either, because the harness measures elements that are IN the document and this one was in the document, fully laid out, just nowhere anybody could look.\\n\\nAND IT WAS THE ONLY MENU ON THE SITE PLACED BY THE STYLESHEET. Every other one goes through attachMenu, which positions in JS off the anchor -- so the fix had to be made twice, and the second half is the more valuable one: attachMenu consulted the ANCHOR and never the VIEWPORT. That is right on a desktop, where no anchor is near an edge, and wrong at 390, where a 247px menu left-aligned under a button at 300 hangs half of itself into nothing. The corner-flipping note in that function has claimed since it was written that the menu "always opens into the screen rather than out of it". It was true of the four corners it was written for and enforced nowhere. It is now a clamp: one rule, both axes, all four corners, and a flip would not have been enough anyway because at 320 both sides of an anchor can be too narrow to hold the menu.\\n\\nTHE ROWS ARE THE REFERENCE, and the cuts are what make it a phone menu rather than a squeezed desktop one. Out: Full screen, picture-in-picture, Backgrounds and effects, Use a phone for audio -- four desktop-window affordances with no window to act on, one of them wanting a camera this build deliberately never opens. In: captions, In-call messages, Meeting tools, Host controls, which are separate controls on a desktop and have nowhere to be at 390.\\n\\nMOVING CAPTIONS COSTS NOTHING, AND THE NOTE THAT SAID OTHERWISE WAS WRONG. The stylesheet had argued captions must stay on the bar because it is how the walkthrough is stopped and a stop should be one press. The board had already settled that in the other direction -- Stop talking stops him, and the captions switch did the same thing while telling him nothing. Stop talking is a pill, it is on screen at 390 for as long as he is speaking, and it is one press. Turning captions off silenced him only as a side effect of hiding the strip he speaks through, which is why it was the worse control even while it was the nearer one. Both notes are corrected in place rather than quietly dropped, because the wrong version of that argument is the kind that gets re-derived.\\n\\nHE SWITCHES THEM ON HIMSELF, which is Nam instruction and also the repair for the one thing the move does break: the strip now arrives with no control anywhere near it to explain where it came from. A second and a half of empty room, then captions appear, then he talks. Same principle as N29 making the hand PERFORM the share rather than switching it on -- a page that shows a control being used teaches it, and one that arrives pre-arranged does not.\\n\\nMINIMIZE IS NOT MERELY DEAD ON A PHONE, IT IS UNREACHABLE. It goes live in exactly one state -- small tile, presenting, unpinned -- and at 390 that state does not exist, because the self tile is a fixed corner thumbnail that cannot be dragged and has nowhere to collapse to. A menu whose first row can never do anything reads as broken rather than contextual, which is what Nam was looking at.\\n\\nTHE RESTLESSNESS METER MEASURES SOMETHING A PHONE DOES NOT DO. It is a reaction rather than a HUD: it flashes when the score goes UP, which happens when a reader bolts out of a section they had barely started, which is a mouse-and-scrollbar behaviour. Swiping through a column barely moves it, so what is left is an unexplained coloured bar over the densest screen in the build, taking room from the captions. Nam: "it doesnt add anything."\\n\\nTWO THINGS FLAGGED RATHER THAN FIXED. The 9-dot glyph now appears twice in one menu, on Adjust view and on Meeting tools, because those are the glyphs both of those things already use and inventing a third is the mistake Round 5 made -- the labels carry the difference, but it does look like a repeat. And the three panels those new rows open have no mobile layout of their own yet; the rows reach real panels, and those panels are still desktop-shaped.',
    },
  },

  {
    id: 'N252', col: 'review', size: 'M', tag: 'call',
    title: 'The phone bar is measured off the real one, not off a screenshot of it',
    note: 'Six controls at 54 by 44. Three of the four numbers were not what the picture suggested.',
    detail: {
      why: 'Nam: "now that the CC button is gone, lets restyle this bottom bar of the mobile view. 2 screenshots, first is ours, second is the original. match it."',
      done: [
        'Every control is 54 by 44 at radius 22, including leave',
        'Gaps of 8, a group of 364, and 13 clear on each side of a 390 screen',
        '16px under the bar, carried by the --botbar token so the eight rules above it follow',
        'The mic and camera lose the muted paint for the device-missing paint the reference uses',
        'Glyphs are 24px, which fixes a shrink that was hitting every stateful control',
        'The 320 floor keeps the same margins and uniform widths at 44 with 6px gaps',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'MEASURED LIVE, NOT READ OFF THE PICTURE, and that mattered three times out of four. The numbers came from meet.google.com in iPhone emulation at 390x844, six controls read straight out of the DOM: 54 wide, 44 tall, radius 22, gaps of 8, group 364, margins 13, bottom gap 16, glyphs 24px in #e3e3e3, leave #db372d.\\n\\nWHAT I WOULD HAVE GOT WRONG BY LOOKING. They read as CIRCLES and they are not -- 54x44 at radius 22 is a stadium with a 10px straight section. I would have sworn LEAVE WAS WIDER than the rest, the way it is on the desktop; on a phone it is the same box in a different colour, and ours was 60 against 44, which is most of why the row looked unbalanced. And the gap I had was 4 against 8, which reads as tight rather than as wrong.\\n\\nOUR MIC AND CAMERA WERE ROUNDED SLABS WITH A FLAT LEFT EDGE, which was a leftover and not a decision. They live inside .unit with a caret and carry `border-radius: 0 r r 0` so they can butt against it; hiding the caret in N250 left the stub geometry behind, square-cornered on the side where there is no longer anything to meet.\\n\\nTHE PINK WAS THE WRONG STATE, NOT THE WRONG COLOUR. #f9dedc at radius 12 is Meet muted paint and it is correctly measured. But the reference machine has no microphone and no camera, so its two controls sit in the DEVICE-MISSING state: dark like everything else, radius 22, and a yellow warn badge. That is our state too, and more literally than the reference -- this build never calls getUserMedia at all, which is a promise the title card makes. A page that cannot open a microphone cannot mute one, so painting these muted described a state the page has no way to be in. The badge was not new work either: warnBadge has been in gm3 since the pre-join chips and .ctl-warn was already mounted on both controls, with one rule further down the file hiding it whenever the control was .off.\\n\\nAND MEASURING BOTH SIDES FOUND A BUG THAT WAS OURS ALL ALONG. The reference glyphs are 24px; ours came back 22. The disagreement turned out to be inside our own function: cbtn builds its icon at 24, the comment two lines below says the measurement is 24, and then sync() rebuilt it at 22. sync runs at mount for every stateful control, so the mic, the camera, the captions and the hand have been drawing two pixels under their measured size for as long as that line has existed, and two pixels under the neighbours that have no state to sync. The leave button kept a 22 of its own for a different reason -- it is hand-built rather than going through cbtn. Both now 24, on every screen size, which is a desktop fix as much as a phone one.\\n\\nTHE TOKEN, NOT THE BOX, for the 16px. Eight other rules measure off --botbar: the caption strip lift, the solo tile floor, the quest tray, the hand pill. Setting `.bar { height }` and leaving the token alone would have moved the bar and left all eight of them 4px out.\\n\\nTHE FLOOR IS THE SAME BAR SHRUNK, not a different one. 320 has 294 to give once the reference 13px margins are kept, and six controls at 44 with 6px gaps come to exactly 294. The height does not shrink with it: 44 is the reference height, it is also the smallest tap target worth having, and the row is short of horizontal room rather than vertical.\\n\\nTWO THINGS NOT CLAIMED. Whether mobile Meet paints a WORKING mic pink when muted is unmeasured -- that machine has no microphone, so the control cannot be toggled and the muted variant never renders; the desktop measurement stands and the mobile one is untaken. And our leave red is #dc362e against the reference #db372d, one step per channel, which is the shared --danger token and not worth forking per breakpoint.',
    },
  },

  {
    id: 'N253', col: 'review', size: 'L', tag: 'call',
    title: 'On a phone the walkthrough was narrating the CV over a call with nothing on it',
    note: 'The share button is hidden below 600px. q() still found it, so every step after it failed politely.',
    detail: {
      why: 'Nam: "Im in the call view, there is no mouse clicking CC, there is no mouse moving to open the more option clicking share screen etc. In fact, the only time the mouse moves is when I disable CC. Double check if the mouse control sequence works on mobile, why isnt the mouse moving, and why things start without any mouse click/movement?"',
      done: [
        'The share is performed on a phone too: overflow, then Share screen, then the desktop as before',
        'Captions are turned on by the hand pressing the control, not by a timer writing state',
        'doShare picks its route from whether the bar button has a box, not from a width',
        'The overflow button has a data-ctl, and the two rows the tour presses have a data-row',
        'Traced end to end at 390 and at 1512, so both routes are observed rather than assumed',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'THE SYMPTOM WAS A STILL CURSOR. THE DEFECT WAS THAT THE CV NEVER GOT SHOWN. Traced at 390: the hand appeared at 2.8s and did not move again for the rest of the run, and the picker, the shared screen and the mock desktop never appeared at all. So the tour said "First, let me get my screen up", and then "This is the CV", and then seven years of experience, over a call with a blank stage. Nam noticed the cursor; what was actually broken was the payload.\\n\\nONE LINE CAUSED IT, and it is a trap worth naming. doShare starts with `pressSel([data-ctl=present])`, and N250 hid that button below 600px because the reference bar has no share control. `display: none` leaves an element PERFECTLY FINDABLE by querySelector and completely unpressable -- so pressSel found it, reported success, and every step after it failed against a picker that was never going to open. Each of those steps gives up quietly on purpose, which is right when a visitor has cancelled a dialog mid-sequence and is exactly wrong when the target was never reachable: the design turned a broken selector into silence. The guard is now a BOX test rather than a media query, so the route follows what is on the screen instead of a number that has to be kept in step with the stylesheet.\\n\\nAND THE CAPTIONS WERE WORSE, because they worked. N251 turned them on with a direct dispatch on a 1400ms timer, which produced the right state at the right moment and no performance whatsoever -- the strip blinked into being while the cursor sat still. The trace is blunt about it: captions flipped on at t=2.1 and the hand did not EXIST until t=2.8. It was a state change wearing the costume of a demonstration, and the whole point of N29 is that this build performs its controls rather than setting them.\\n\\nSO IT IS A CUE NOW. The stage owns the hand, so anything the hand should do belongs in a cue and not in a timer in the call screen. `captions` opens the overflow and presses the row, tests the STATE rather than the viewport -- captions ship on, so on a desktop it returns having touched nothing -- and sits on line 0 of the intro. What the visitor sees at 390: an empty call, the cursor arriving, the overflow opening, Turn on captions pressed, and then he starts talking. Which is what was asked for in N251 and was not what N251 built.\\n\\nTHE ROWS NEEDED HANDLES. The hand can only aim at a selector, and the only other way into a menu row is its label -- which is written for a reader and changes with state, "Turn on captions" against "Turn off captions". A script that matched on that would break the first time the wording moved. So MenuItem takes an optional key, rendered as data-row, on the two rows the tour actually presses. The overflow button had no data-ctl at all, which was harmless while nothing needed to find it.\\n\\nMEASURED AFTERWARDS, at 390: hand to the overflow at 21.0, menu open at 21.7, Share screen pressed at 22.4, shared desktop up at 23.1, then Chrome launched and maximised as on the desktop. At 1512 the picker route is untouched -- picker open from 12.6 through 15.4, share up at 16.1, and the overflow never involved.\\n\\nONE THING THAT LOOKS WRONG AND IS NOT: between beats the hand sits at x 451 on a 390 screen. That is park(), which goes deliberately to innerWidth + 60 and drops is-on -- off the stage and hidden, which is the correct place for a cursor with nothing to do.',
    },
  },

  {
    id: 'N254', col: 'review', size: 'S', tag: 'call',
    title: 'A cue that presses a toggle has to assert the state, not the press',
    note: 'Press means invert whatever is true when it lands. The cue wanted captions ON.',
    detail: {
      why: 'Nam: "it seems that the CC was on by default, then when the mouse click CC, it actually disables it LOL. Fix it"',
      done: [
        'The captions cue reads the state immediately before the press, not before the travel',
        'It bails without touching the row, and closes the menu, if captions came on meanwhile',
        'It checks the outcome and goes round once more, re-opening the overflow to do it',
        'Traced through the green room at 390, 412 and 1512',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'A PRESS ON A TOGGLE IS NOT AN INSTRUCTION, IT IS AN INVERSION. N253 wrote the captions cue as read-the-state, travel to the row, press -- and the travel is the best part of a second. Anything that moved captions during it turned the press into its own opposite: the visitor pressing the row themselves, the `c` shortcut in main.ts, Settings, or the store settling later than the read. A cue that is only correct while nothing else happens during it is not correct, and this one was reaching for the single control the visitor is most likely to be poking at -- Nam had said in the same session that disabling CC was the one thing he had been doing by hand.\\n\\nSO IT IS AN ASSERTION ABOUT THE END STATE NOW. Read late rather than early. Bail without touching the row if the work has already been done, closing the menu that was opened for it (N157). Then check the outcome, because looking is the only way to know what a toggle did.\\n\\nTHE RETRY HAD TO BE A LOOP, and the first version of it was dead code dressed as a safety net. A pick closes the menu, so by the time the outcome can be checked the row is gone from the document, and the bar captions button is display:none on the only screen this runs on. hand.at() rightly refuses a target with no box, so a retry reaching for either would have quietly done nothing. Going round the loop re-opens the overflow first, which is also what a person would do.\\n\\nNOT REPRODUCED ON A FRESH BUILD, and that is stated rather than glossed: traced through the green room at 390, 412 and 1512, captions read false, the row read "Turn on captions", the press landed, and all three sources of truth agreed afterwards. Desktop no-ops correctly -- the cc-on class is applied at mount, so the cue returns before it opens anything. What is fixed is the mechanism that produces exactly the reported symptom, which is the honest thing to fix when the report is credible and the repro is not in hand.',
    },
  },

  {
    id: 'N255', col: 'review', size: 'L', tag: 'call',
    title: 'The phone gives the screen to the share, and the captions stop being a card',
    note: 'The share was 279 of 390. The 80 percent zoom had never once applied. Both found by measuring.',
    detail: {
      why: 'Nam: "the screensharing view can be wider and taller, and the mock OS needs to be tidied up on mobile: we hide the temperature, wifi and volume symbols. And icons on task bar is to align to the left." And: "the view for the shared screen + CC is very busy ... it looks nothing like us, not a box that pops out ... scripted transcript, no audio => remove all this. text position: move in lower and put the stop talking above it." Plus captions on by default, the progress bar on mobile, a shorter emoji row, and a stop control that only exists while there is talking to stop.',
      done: [
        'The share goes from 279x360 to 374x468, and the mock OS fills all of it',
        'The 80 percent zoom applies for the first time, through zoom rather than transform',
        'The taskbar loses the weather and the tray cluster, and aligns its icons left',
        'The caption is text on the screen rather than a card, with the label hidden',
        'Stop talking sits above the caption, and only while there is talking to stop',
        'The walkthrough progress bar comes back on a phone, above the controls',
        'Seven reactions instead of ten',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'THE MISSING 111 PIXELS DID NOT BELONG TO THE SHARE. Measured before touching anything: .shot-wrap and .shot were both 279 by 360, and both were simply filling their parent. The constraint was the rule on .grid-wrap.is-solo, inset 72px 56px 80px 55px -- real measurements off the desktop original, where 55px of surround is a margin and at 390 is 28 percent of the screen spent on two black bars. 8px a side instead. With the band below re-cut as well the share is 374 by 468, half again the area.\\n\\nAND THE 80 PERCENT ZOOM HAD NEVER APPLIED. The rule from N250 selected .shot .dk, and THERE IS NO .dk -- the mock OS is a .pg.pg-desk holding the wallpaper, .dk-surface and .dk-taskbar, every class in that subtree dk- prefixed and none of them bare dk. So the zoom-out reported in that round did nothing at all, and what improved was the aspect change alone. Worth recording as a class of mistake rather than a typo: a selector that matches nothing fails EXACTLY like a rule that is being overridden, the page looks unchanged either way, and the deadcss gate did not catch it either. Same lesson the specificity traps keep teaching -- measure the element afterwards, do not read the rule and believe it.\\n\\nTHEN THE FIX TOOK THREE GOES AND THE OLD COMMENT WAS BACKWARDS. All three measured:\\n\\n  125 percent + scale(0.8)   box 467.5 x 468, painted 374 x 374\\n  125cqw / 125cqh + scale    box 467.5 x 468, painted 374 x 374\\n  zoom 0.8                   box 467.5 x 585, painted 374 x 468\\n\\nOnly the last fills the frame. Both compensating versions won the width and lost the height -- 468 rather than 585 either way -- so the desktop laid out at the height of the frame and painted at 80 percent of it, leaving a 95px grey band under the taskbar exactly where the extra desktop was supposed to be. The note in the stylesheet had argued AGAINST zoom, on the grounds that it would re-resolve percentage heights inside the desktop and leave a gap at the bottom. That argument was written for the selector that never matched, so it had never been tested against the page, and the gap it predicted is precisely what the transform version produced.\\n\\nTHE TASKBAR: the weather and the quick settings cluster are the two things on it that say nothing about this CV, and they are what the harness had been reporting as a tray-over-icons overlap for several passes. The clock stays, because it is the one tray item doing work -- the desktop clock and the call clock are deliberately the same time. Centred pinned icons are Windows 11 and correct on a wide bar; on a narrow one, centring puts the icons where the tray wants to be.\\n\\nTHE CAPTION WAS A SECOND CARD UNDER A CARD. A 358px black rectangle with its own radius, directly beneath a shared screen that is also a rectangle with its own radius, where the reference has one picture and some words under it. The background and the padding go, and nothing legible is lost: the call behind is near black already and the text is 18px white on it. The lift goes from 116 to 46, and that number had simply stopped meaning anything -- 116 exists to clear the self-view tile, which above 900px sits BESIDE the caption. On a phone the tile is a strip under the share and there is nothing beside the caption at all, so the lift was buying clearance from something that had moved, on the screen with the least room to spare.\\n\\nThe honesty label had to be split out to be hidden. The phrase about a scripted transcript with no audio answers a real objection in the devlog -- captions implying speech recognition where there is no audio would be a lie -- so it stays wherever there is room for it, and CSS cannot hide half a text node. The name and the label are two elements now.\\n\\nTHE BAND CARRIES THREE THINGS, bottom to top: progress bar, caption, Stop talking. Nam put the readout nearest the controls on exactly the right grounds -- "this one is not interactable so we can have it above the bottom line ... stop talking for example, if too close to the mic button, user may misclick." The unpressable thing goes where a mis-tap is harmless, and the button goes 130px up where a mis-tap cannot happen.\\n\\nAND THE STOP CONTROL ANSWERS THREE SEPARATE QUESTIONS, which is why it was wrong in three ways at once. Has he started at all -- it mounts hidden and voice() lifts it on the first line, because a button offering to stop the talking before any talking has happened is offering nothing. Has the visitor silenced him -- body.cc-on, read straight by the stylesheet so it cannot lag; it used to wait for the stage safe point, which is AFTER the current line and after any beat on it, so a stop pressed under the share waited for the share to finish. Is the flow over -- delivering(), which nothing had been watching on behalf of the bar, so after the last line it sat there offering to stop a conversation that had ended.\\n\\nCAPTIONS ON BY DEFAULT RETIRES TWO ROUNDS OF WORK, and the second was right about the first. N251 started the phone with captions off so the bar could match the reference six controls, and turned them on with a timer -- state with no performance behind it. N253 made it a real cue so the hand pressed the actual control. Both were answering a question that did not need asking: the thing being demonstrated was a control the visitor has no reason to care about yet, and it cost the opening two seconds of watching a menu open. The share is the demonstration that earns its time, because the share is what the CV IS.\\n\\nSEVEN REACTIONS, NOT TEN. The first seven come from Meet itself; the last three are ours, one per thing he actually did, and they are the ones to cut precisely BECAUSE they are ours -- the tray was running past the edge at 390, and a row that has to scroll to reach the standard reactions is worse than one that does not carry the jokes.\\n\\nONE THING LEFT UGLY AND FLAGGED: the quest toast lands on the self-view strip while presenting. Every position on a 390 screen covers something, and the toast is transient and dismissible, so it is noted rather than churned.',
    },
  },

  {
    id: 'N256', col: 'review', size: 'L', tag: 'call',
    title: 'Stop talking finally stops him, and the bottom of the phone stops fighting itself',
    note: 'One hidden element, two rounds apart, had broken two different features the same way.',
    detail: {
      why: 'Nam, nine items after the previous round: Stop talking higher and clear of a three-line caption; browser tabs that fit; the presenting chip as an icon; a clock that does not wrap; Stop talking that actually stops him; folders that show they have something in them, with double click opening rather than renaming; the progress readout clear of the reaction tray and of the self-view tile.',
      done: [
        'Stop talking presses the captions control wherever it lives, so it works on a phone',
        'The band is sized for a three-line caption, so the button never covers the words',
        'The reaction tray lifts the whole stack, and the tile clears the readout with captions off',
        'Mock browser tabs share the strip instead of queueing for it',
        'The presenting chip is its icon, and the clock stays on one line',
        'Folders with contents look like it, and rename is a gesture on the name',
      ],
      raised: 'Nam, 31 Aug',
      notes: 'STOP TALKING WAS THE SAME BUG AS THE SHARE, TWO ROUNDS LATER. Nam: "Stop talking doesnt work ... the mouse was trying to click the caption button but it failed to turn off caption." Exactly the diagnosis. pauseHere goes quiet by pressing [data-ctl=captions]; N251 moved that control into the overflow menu and left the bar button at display:none; hand.at() rightly refuses a target with no box, so the press was a no-op -- and since going quiet is what ENDS the pause sequence, he simply carried on talking. ONE HIDDEN ELEMENT BROKE TWO SEPARATE FEATURES IN THE SAME SHAPE, a fortnight of tickets apart, and both times the failure was silent because every step in these sequences gives up quietly by design. The route is chosen by the box now, the way doShare already does it. The lesson is worth stating plainly: hiding a control with CSS does not hide it from a script, and any script that presses controls needs to ask whether the control is THERE, not whether it EXISTS.\\n\\nTHE BAND WAS SIZED AGAINST THE CAPTION BEST CASE. Nam twice, first "a bit cramped when the caption goes on 2 lines" and then "I found this case where the caption goes on 3 lines. Now stop talking is actually hiding text." Three lines is 18 for the name plus 78 of text, so the caption can reach 226 off the floor and the button has to clear that rather than the single line it usually sits above. It costs the share about 50px, which is the right way round: a control that sometimes covers the sentence it is interrupting is worse than a slightly shorter picture.\\n\\nTWO STATES I HAD NOT ACCOUNTED FOR, both found by Nam rather than by the harness, and both the same omission. The reaction tray opens into the slot directly above the control bar, which is exactly where the progress readout now lives; and with captions OFF there is no band at all, so the tile fell back to an inset written before the readout existed on this screen. The tray case is fixed by lifting the whole stack together rather than the readout alone -- moving only the readout would have parked it on the caption -- and the band variable is what makes that one line.\\n\\nTABS THAT QUEUE RATHER THAN SHARE. `max-width: 200px` with no flex means each tab takes what it wants and the last falls off the strip, which is the half-visible third tab in the screenshots. flex 1 1 0 with min-width 0 is the pair that works: the basis has to be 0 or the line breaks at the hypothetical size before any shrinking happens, and the automatic minimum has to be overridden or each tab stays as wide as its own label. Chrome shrinks its own tabs to fit, so this is the reference behaviour as well as the useful one.\\n\\nA FOLDER THAT HAS SOMETHING IN IT NOW LOOKS LIKE IT. Nam sent two captures of a real Explorer window where the difference is unmistakable -- an empty folder is a plain shape, a full one has a sheet standing up behind its front flap, and the tooltip on the empty one says Empty folder. It is an affordance rather than decoration: every folder drawn identically says nothing about which are worth opening, and on this desktop that is the difference between a visitor finding the clips and not. Read off the same FOLDERS map the count beside it comes from, so the icon and the number cannot disagree.\\n\\nAND RENAME BECOMES A GESTURE ON THE NAME. The old version armed a rename on a second click anywhere on the row and relied on dblclick to cancel it 260ms later. That is the real Windows rule and it worked until it did not: two clicks landing slightly apart fire two clicks and no dblclick, so an open became a rename. Scoping it to the label removes the race instead of tuning it -- the name is a small target inside a wide row, so an open aimed at the icon or the empty space to its right can never arm a rename at all. Nam: "double clicking should open it, not opening rename."\\n\\nMEASURED AFTERWARDS: the clock is one line at 62px, the presenting chip is 36 by 36 with nothing but its glyph, four tabs sit at 82px each inside a 372px strip with 29px to spare, and a press on Stop talking now produces the acknowledgement and takes the captions down about six seconds later, which is the sequence finishing rather than being abandoned.\\n\\nSTILL FLAGGED: the quest toast lands on the self-view strip while presenting. Every position on a 390 screen covers something.',
    },
  },

  {
    id: 'N257', col: 'review', size: 'M', tag: 'call',
    title: 'A gag that fires on a scroll comes out of a script that scrolls',
    note: 'And the completion ring stops asking whether a call was finished.',
    detail: {
      why: 'Nam, watching the CV segment: "the scrolling of the script triggers the pap pap pap gag which then triggers a bunch more scripts and we lost 2 lines not being able to read at all. So what do we do? remove the pap pap pap line, or actually this whole bail on line 1. We are in a script, everything is tightly timed, so this is not helping only hurting the script." And, from an incognito window: "finish the interview early, in the end screen I see the quest button but not the progression bar?? The progression bar should be triggered the moment we make any progress at all."',
      done: [
        'The bail gag is deleted -- data, watch, runner, spec card and styles',
        'The visitor profile keeps its own bail tracking, which speaks no lines',
        'The completion ring shows on any progress, not only after a finished call',
        'Its synchronous gate still keeps the completion chunk out of a fresh visit',
      ],
      raised: 'Nam, 1 Sep',
      notes: 'THE GAG WATCHED FOR A THING THE SCRIPT DOES. N35 armed on the Wasabi line and fired if the visitor scrolled away inside three seconds -- but that part’s own beats scroll the document, so the trigger and the script were competing for the same event. There is a `rolling` flag raised around every programmatic roll for exactly this reason, and it still did not hold: the CV is in an iframe whose scroll events are proxied out to the handler, and the flag drops on a 120ms timer that a proxied event can land after. The gag then fired mid-segment and spoke three lines over the three the script was already saying.\\n\\nDELETED RATHER THAN TUNED, and the arithmetic is the argument. Tightening the window is guessing at a race whose downside is losing the script’s own lines and whose upside is a three-second joke. Nam had already reached the same place from the other end: the gag is not helping, only hurting. What survives is the profile’s bail tracking, which is a different thing wearing the same word -- it counts sections left in under three seconds to work out how fast this visitor moves, and it never speaks.\\n\\nTHE RING ASKED THE WRONG QUESTION, and the Quests item beside it is what made that visible. `recordInterview` is called from one place, the tour’s `finished` callback, so leaving a call early never writes it -- and the ring was gated on that record rather than on progress. A visitor who found three things and left had a rail that had already decided they had none, with Quests sitting next to the gap. N118’s actual worry was a first-time visitor being handed a scoreboard for a game nobody mentioned, and that is a worry about ZERO: the `pct <= 0` check underneath had been answering it all along, while the interview gate stood in front doing a different job badly.\\n\\nTHE SIZE GATE SURVIVES, which is the only reason the interview check was there in the first place. foundAll() reads localStorage and nothing else -- the collections its ids would be counted against live in completion.ts, which is what the deferred chunk carries -- so "has this visitor found anything" is answerable synchronously and a fresh visit still fetches nothing. Quips are deliberately left out of that sum: the ring does not count them, so they must not open the door to it.\\n\\nMEASURED: a fresh profile on the ended screen shows Meetings and Calls and no ring; the same profile with one quest and one bug found, and no interview record at all, shows Quests and a 3% ring.',
    },
  },
