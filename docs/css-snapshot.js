...
(() => {
  ...
  window.__cssSnap = async () => {
    ...
    /*
     * Which classes the stylesheet talks about but no state ever produces.
     *
     * This is the OTHER kind of dead CSS, and the prune tool cannot see it. That
     * one deletes rules whose value is overridden; this finds rules whose
     * selector never matches anything, which is what happens when markup is
     * rewritten and the old rules are left behind. Rebuilding the achievement
     * toast orphaned .quest-row, .quest-name, .quest-sub and .quest-x in one
     * commit, and nothing in the build would ever have complained.
     *
     * Treat the output as candidates, not a verdict. A class can be absent here
     * and still live — anything behind a viewport the harness does not visit, a
     * device the harness does not have, or a state it does not reach. Every
     * deletion still gets checked against the source by hand.
     */
    const seenClasses = new Set();
    for (const key of Object.keys(sig)) {
      for (const seg of key.split(' ').slice(1).join(' ').split('>')) {
        for (const cls of seg.split('.').slice(1)) if (cls) seenClasses.add(cls.split(':')[0]);
      }
    }
    window.__seenClasses = [...seenClasses].sort();
    const perState = {};
